    ...
    function processExpression(node) {
      switch(node.type) {
        ...
        case "MemberExpression":
          ...
          var p
          if(node.computed) {
            p = processExpression(node.property)
          } else {
            p = new Literal(node.property.name, node.property)
          }
          emitGet(r, o, p, node)
          return clone(r)
        break
        ...
        case "UnaryExpression":
          if(node.operator === "delete") {
            if(node.argument.type === "MemberExpression") {
              ...
              var r = temporary(node)
              var p = new Literal(node.argument.name, node.argument)
              var pendingIfs = []
              var isBound = false
outerFor:     for(var cenv = env; cenv; cenv=cenv.parent) {
                if(cenv.withObject) {
                  emitDelete(clone(r), 
                    new VariableId(cenv.withObject, node), 
                    clone(p), 
                    node)
                  var ift = new IfTerminator(clone(r), 0, 0, node)
                  pendingIfs.push(ift)
                  splitBlock(ift)
                  ift.alternate = cblock.id
                }
              ...
              if(isBound) {
                assign(clone(r), new Literal(false, node), node)
              } else {
                emitDelete(clone(r), 
                  new VariableId("~global", node), 
                  clone(p), 
              ...
              for(var i=0; i<pendingIfs.length; ++i) {
                pendingIfs[i].consequent = cblock.id
              }
              return clone(r)
            } else {
              var r = temporary(node)
              var arg = processExpression(node.argument)
              ops.push(new UnaryOperator("!", r, arg, node))
            ...
            var r = temporary(node)
            var arg = processExpression(node.argument)
            ops.push(new UnaryOperator(node.operator, r, arg, node))
            return clone(r)
          }
        break
        ...
        case "AssignmentExpression":
          ...
          if(node.left.type === "MemberExpression") {
            ...
            if(node.operator === "=") {
              ...
              ops.push(new BinaryOperator(tok.substr(0, tok.length-1), tmp, b, a, node))
        ...
        case "CallExpression":
          ...
          if(node.callee.type === "MemberExpression") {
            ...
            if(node.callee.computed) {
              p = processExpression(node.callee.property)
            } else {
              p = new Literal(node.callee.property.name, node.callee)
            }
            emitGet(clone(f), clone(o), p, node.callee)
    ...
    function processStatement(stmt, label) {
      ...
      switch(stmt.type) {
        ...
        case "SwitchStatement":
          ...
          switchBreak = switchNext = cblock.id
        ...
        case "ForStatement":
          ...
          var jmpUpdate = new JumpTerminator(loopTest, stmt.update || stmt)
          ...
          ifTest.consequent = processBlock(stmt.body, makeEnv({
            next: loopUpdate,
